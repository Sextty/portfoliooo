...
| 2026-07-18 | **Three new flagship SaaS platforms, fully productionized:** Cadence (AI workspace — glassmorphism, ⌘K command palette, AI assistant with real task-creation side effects, kanban, calendar), Ledgerly (finance analytics — dark corporate, searchable/sortable/paginated transaction table with CSV export, budgets, spending insights computed live from data), Aurawell (healthcare — light emerald, patient portal + doctor console sharing one appointment ledger, multi-step booking flow, DB-level double-booking prevention). Each ships as: (1) a deep interactive in-browser demo at `/demo/<id>` (this repo), (2) a full product-page case study (problem/solution/architecture diagram/decisions/roadmap in `caseStudies.ts`), (3) a genuinely real standalone full-stack repo under `Desktop/projects/<id>` — Express + Prisma + PostgreSQL + JWT auth + Docker + seed data + README + MIT license, verified via `npm run build` (frontend), `node --check` (every backend file), and `prisma validate` (every schema). `src/data/projects.json` bumped to version 4 (11 projects total); new `AppMockup` variants (GlassWorkspaceMock, LedgerMock, CareMock); catalog's AI-powered category matcher broadened beyond the `OpenAI` tag to also catch title/tagline/description mentions of "AI". |
